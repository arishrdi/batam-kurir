...
$(document).ready(function () {
    ...
    /* Point Reward */
        $('#point_reward').submit(function(e) {
            $('#submit').prop('disabled', true);
            e.preventDefault();
            $.ajax({
                url: 'proses/update/reward.php',
                type: 'POST', 
    ...
    /* Pick UP */ 
        ...
        $('#update-pickup').submit(function(e) {
            var payload = $('#payload').val();
            $('#submit_edit').prop('disabled', true);
            e.preventDefault();
            $.ajax({
                url: 'proses/update/pickup.php',
                type: 'POST', 
                data: new FormData(this),
                contentType: false,
                processData: false,
                success: function(data) {
                    ...
                    console.log(data)
                        ...
                        Toast.fire({
                            icon: 'error',
                            title: 'Update Failed',
                            text: 'Tidak dapat memperbarui record', 
                        });
                    }
                    setTimeout(function(){
                        window.location.href='pick_up.php'+payload;
                    }, 1500);
                }
            })
        });
